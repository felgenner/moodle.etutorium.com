...
<div style="float: left; width: 100%;" id="<?php echo $data['id']; ?>">
    <table style="width: 100%" class="admintable generaltable">
        ...
        <tr>
            ...
            if ($data['action'] == 'add') {
                echo  get_string('webinarsnotfound', 'etutorium');
            } else {
                echo get_string('addwebnotfound', 'etutorium');
            }
            ?>
            </td>
        </tr><?php
        ...
        ?>
        <tr id="<?php echo $data['id'].$webinar['id']; ?>">
            <td style="text-align: center; cursor:pointer;" onclick="webinaredit.viewmore(<?php
            echo $webinar['id']; ?>, '<?php
            echo $data['id']; ?>')"><?php echo $webinar['id']; ?></td>
            <td style="cursor:pointer;" onclick="webinaredit.viewmore(<?php
            echo $webinar['id']; ?>, '<?php
            echo $data['id']; ?>')"><?php echo $webinar['title']; ?></td>
            <?php if ($data['id'] == 'allweblist') {
                ?><td style="text-align: center; cursor:pointer;" onclick="webinaredit.viewmore(<?php
                echo $webinar['id']; ?>, '<?php
                echo $data['id']; ?>')"><?php echo get_string($webinar['status'], 'etutorium'); ?></td><?php
} else {
                ?><td style="text-align: center; cursor:pointer;" onclick="webinaredit.viewmore(<?php
                echo $webinar['id']; ?>, "<?php echo $data['id']; ?>')">
                    <a href="./connect.php?webinar_id=<?php
                    echo $webinar['webinar_id']; ?>&id=<?php echo $data['etutorium_id']; ?>" target="_blank"><?php
                    echo (!$diff->invert) ? get_string('connect', 'etutorium') : get_string('get_archive', 'etutorium'); ?>
                    </a>
                </td><?php
} ?>
            <td style='text-align: center;'><input type="button" value=" <?php
            echo ($data['action'] == 'add') ? '+' : '-'; ?> " onclick="webinaredit.<?php echo $data['action']; ?>Webinar(<?php
            echo $webinar['id']; ?>)"></td>
        </tr>
        <?php
            }
