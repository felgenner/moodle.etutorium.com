...
<?php foreach ($data['data']['regfields'] as $field) { ?>
    <tr>
        <td style="width:40%; text-align: right;"><?php echo $field['field_name'].required($field['required']); ?></td>
        <td style="width: 5%;"></td>
...
?>
    <tr>
        <td colspan="3" style="text-align: center;"><input type="button" value="<?php
        echo get_string('connect', 'etutorium'); ?>" onclick="webinarconnect.webinarConnect('<?php
        echo $data['webinarid']; ?>')"></td>
    </tr>
</table>
...
function required($required) {
    if ($required) {
        return '<img class="req" title="'.get_string('requiredelement', 'form').'" '
            . 'alt="'.get_string('requiredelement', 'form').'"'
            . ' src="http://moodle24.com/theme/image.php/standard/core/1454501321/req">';
    } else {
        return '';
    }
}
...
function typefield($field) {
    ...
    switch (strtolower($field['field_type'])) {
        case 'area':
            $return = '<textarea maxlength="'.$field['field_length'].'" name="field'.$field['id'].'" '
            . 'id="field'.$field['id'].'">'.$field['value'].'</textarea>';
            break;
        default :
            $return = '<input type="text" maxlength="'.$field['field_length'].'" name="field'.$field['id'].'" '
            . 'id="field'.$field['id'].'" value="'.$field['value'].'" />';
            break;
    }
    return $return;
}